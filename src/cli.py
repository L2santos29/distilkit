...
def build_parser() -> argparse.ArgumentParser:
    """Configure argparse with subcommands: train, benchmark, export, gui, api.

    ``gui`` is the default subcommand when none is given.  Every subcommand
    has sensible defaults so ``distilkit train`` works out of the box.
    Help is auto-generated by argparse from the argument definitions below.
    """
...
def cmd_train(args: argparse.Namespace) -> nn.Module | None:
    """Execute the entire train → benchmark → export workflow via ``run_distillation_pipeline``.

    Logs the resolved configuration to stdout, delegates to the shared
    pipeline (used by both CLI and web GUI), and exits with code 1 if
    the pipeline raises a ``PipelineError`` (e.g. download failure,
    unsupported dataset).  Returns the trained student model on success.
    """
    logger.info("=" * 60)
...
def cmd_benchmark(args: argparse.Namespace) -> None:
    """Benchmark a trained model on the given target device.

    Dispatches to ONNX Runtime for ``.onnx`` files (with optimized thread
    settings) or PyTorch for ``.pt``/``.pth`` files (with optional state-dict
    reconstruction).  All I/O and model-load errors are caught and logged
...
def cmd_export(args: argparse.Namespace) -> None:
    """Export a trained PyTorch checkpoint to ONNX or TorchScript.

    Reconstructs a ``MiniCNN`` student with the default 10-class head,
    loads the state dict from ``args.model`` (supporting both raw state
    dicts and wrapped ``{"state_dict": ...}`` checkpoint files), and
    dispatches to the appropriate export utility.
    """
...
def main() -> None:
    ...
    When no subcommand is given (or ``gui`` is requested), the web GUI
    is launched.  If the GUI dependencies (FastAPI, uvicorn) are missing,
    a clear error instructs the user to install requirements.
    """
    parser = build_parser()
